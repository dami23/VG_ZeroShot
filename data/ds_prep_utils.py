...
class BaseCSVPrepare(ABC):
    ...
    '''
    @abstractmethod
    def get_trn_val_test_ids_unc(self, output_annot=None) -> List[Any]:
        """
        Obtain training, validation and testing ids.
        Depends on the dataset
        """
    ...
    @abstractmethod
    def get_trn_val_test_ids_umd(self, output_annot=None) -> List[Any]:
        """
        Obtain training, validation and testing ids.
        Depends on the dataset
        """
    ...
    def get_df_from_ids(self, ids: List[Any], annots: DF, split_type='val'):
        ...
        if split_type == 'train':
            if self.ds_prep_cfg.root == './data/refcoco' or self.ds_prep_cfg.root == './data/refcoco+' or self.ds_prep_cfg.root == './data/refcocog':
                assert len(annots_to_use['query']) == len(annots_to_use['img_id'])
                
                annots_lines = []
                for i in range(len(annots_to_use['query'])):
                    img_id = annots_to_use['img_id'].values[i]
                    bbox = annots_to_use['bbox'].values[i]
                    queries = annots_to_use['query'].values[i]

                    for j in range(len(queries)):
                        query = queries[j]
                        annots_line = img_id, bbox, query
                        annots_lines.append(annots_line)
                        # 
                return pd.DataFrame(annots_lines, columns=['img_id', 'bbox', 'query'])
            else:
                print ('*'*20)
    ...
    def save_annot_to_format(self):
        ...
        if self.ds_prep_cfg.root == './data/refcoco' or self.ds_prep_cfg.root == './data/refcoco+':
            trn_ids, val_ids, testA_ids, testB_ids = self.get_trn_val_test_ids_unc(output_annot)
            output_annot = output_annot[['img_id', 'bbox', 'query']]
            trn_df = self.get_df_from_ids(trn_ids, output_annot, split_type='train')
            ...
            if testA_ids is not None:
                testA_df = self.get_df_from_ids(testA_ids, output_annot)
                testA_df.to_csv(self.csv_root / 'testA.csv', index=False, header=True)

            if testB_ids is not None:
                testB_df = self.get_df_from_ids(testB_ids, output_annot)
                testB_df.to_csv(self.csv_root / 'testB.csv', index=False, header=True)

        elif self.ds_prep_cfg.root == './data/refcocog':
            trn_ids, val_ids, test_ids = self.get_trn_val_test_ids_umd(output_annot)
            output_annot = output_annot[['img_id', 'bbox', 'query']]
            trn_df = self.get_df_from_ids(trn_ids, output_annot, split_type='train')
            trn_df.to_csv(self.csv_root / 'train.csv', index=False, header=True)
            ...
            if test_ids is not None:
                ...
                test_df.to_csv(self.csv_root / 'test.csv', index=False, header=True)

        elif self.ds_prep_cfg.root == './data/flickr30k':
