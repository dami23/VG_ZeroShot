...
class ReferItCSVPrepare(BaseCSVPrepare):
    ...
    def get_annotations(self):
        self.instance_dict_by_ann_id = {v['id']: ind for ind, v in enumerate(self.ref_inst_ann)}
    ...
    def get_trn_val_test_ids_unc(self, output_annot: DF):
        trn_ids_mask = output_annot.split.apply(lambda x: x == 'train')
        val_ids_mask = output_annot.split.apply(lambda x: x == 'val')
        testA_ids_mask = output_annot.split.apply(lambda x: x == 'testA')
        testB_ids_mask = output_annot.split.apply(lambda x: x == 'testB')
        
        return trn_ids_mask, val_ids_mask, testA_ids_mask, testB_ids_mask
    
    def get_trn_val_test_ids_umd(self, output_annot: DF):
        trn_ids_mask = output_annot.split.apply(lambda x: x == 'train')
        val_ids_mask = output_annot.split.apply(lambda x: x == 'val')
...
if __name__ == '__main__':
    ...
    ref = ReferItCSVPrepare(ds_cfg['refcocog'])
