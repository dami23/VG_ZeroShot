...
class FlickrCSVPrepare(BaseCSVPrepare):
    ...
    def get_trn_val_test_ids(self, output_annot=None):
        trn_ids = list(pd.read_csv(self.ds_prep_cfg.trn_img_ids, header=None)[0])
        val_ids = list(pd.read_csv(self.ds_prep_cfg.val_img_ids, header=None)[0])
        test_ids = list(pd.read_csv(self.ds_prep_cfg.test_img_ids, header=None)[0])
