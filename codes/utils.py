...
@dataclass
class DataWrap_RefCOCO:
    path: Union[str, Path]
    train_dl: DataLoader
    valid_dl: DataLoader
    testA_dl: Optional[Union[DataLoader, Dict]] = None
    testB_dl: Optional[Union[DataLoader, Dict]] = None
...
class Learner:
    uid: str
    cfg: Dict
    data: DataWrap_OCID # DataWrap_RefCOCO
    mdl: nn.Module
    loss_fn: nn.Module
    eval_fn: nn.Module
    opt_fn: Callable
    ...
    def validate(self, db: Optional[DataLoader] = None,
        ...
        with torch.no_grad():
            ...
            for batch in progress_bar(db, parent=mb):
                ...
                predicted_box_dict_list += self.get_predictions_list(prediction_dict)
