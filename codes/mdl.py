...
class BackBone(nn.Module):
    ...
    def __init__(self, encoder: nn.Module, cfg: dict, out_chs=256):
        ...
        self.Linear = nn.Linear(1024, 256) # 1024 for RN50, 768 for RN50x16, 512 for RN101
    ...
    def np_attention(self, x, emb):
        # emb_proj = self.Linear(emb).unsqueeze(2)
        # emb_proj_expand = emb_proj.expand(emb.size(0), x.size(1), x.size(2)).unsqueeze(-1)
        # multi_mat = torch.matmul(x, emb_proj_expand)

        emb_proj = self.Linear(emb).unsqueeze(2).unsqueeze(3)
        multi_mat = x * emb_proj
        multi_attn = self.softmax(multi_mat)
        feat_weighted = x * multi_attn
    ...
    def forward(self, inp, we=None, np_emb=None,
        ...
        out_query = [self.concat_we(f, we, only_we=only_we, only_grid=only_grid) for f in feats]
        
        # noum phrase guided attention
        if not self.cfg['vis_context']:
            return out_query
        else:
            assert np_emb is not None
            out_np =[self.np_attention(f, np_emb) for f in feats]
            out = [torch.cat((out_query[i], out_np[i]), dim = 1) for i in range(len(out_query))]
            return out
...
class ZSGNet(nn.Module):
    ...
    def __init__(self, backbone, n_anchors=1, final_bias=0., cfg=None):
        ...
        if self.cfg['use_lang'] and self.cfg['use_img']:
            self.start_dim_head = self.lstm_dim*(self.bid+1) + 2 + 512  # + 256 for RN50 (1538), RN50x16 (1282), RN101 (1026)
    ...
    def apply_lstm(self, word_embs, qlens, max_qlen, get_full_seq=False):
        ...
        # Packed Embeddings
        packed_embed_inp = pack_padded_sequence(embeds, lengths=qlens1.cpu(), batch_first=False)
        ...
        lstm_out, req_lens = pad_packed_sequence(lstm_out1, batch_first=False, total_length=max_qlen)
            
        # TODO: Simplify getting the last vector
        masks = (qlens1-1).view(1, -1, 1).expand(max_qlen, lstm_out.size(1), lstm_out.size(2))                                
    ...
    def forward(self, inp: Dict[str, Any]):
        ...
        # see full language + image (happens by default)
            ...
            feat_out = self.backbone(inp0, req_emb, np_emb)
        
        # Strategy depending on shared head or not
        if self.cfg['use_same_atb']:
            att_bbx_out = torch.cat([self.permute_correctly(self.att_reg_box(feature), 5) for feature in feat_out], dim=1)
